...
function SigninButton({ pathname }) {
 
  return (
    <form action={signInAction}>
      <input type="hidden" name="pathname" value={pathname || ""} />
      <button className="flex items-center justify-center gap-3 w-64 border max-md:bg-white/80 border-gray rounded-md py-2 px-4 text-gray hover:bg-grayLight transition">
        <svg
          className="w-5 h-5"
          viewBox="0 0 48 48"
        ...
        <span className="text-sm md:text-base max-md:text-black">
          Sign up with Google
        </span>
      </button>
    </form>
