...
import { getGuestById } from "../_lib/data-service";
import SelectCountry from "./SelectCountry";
import SubmitButton from "./SubmitButton";

async function CheckoutForm({ orderId, totalPrice }) {
  const session = await auth();
  const info = await getGuestById(session?.user?.guestId);
  const { country, address, city, postal_code, show_data, name } = info;
  // console.log(info)
  ...
  return (
    <form
      action={pay}
      className="flex flex-col gap-4 w-full px-5 max-sm:order-2"
    >
      <h2 className="text-[46px] font-volkhov max-sm:text-[32px]">Delivery</h2>

      <SelectCountry country={country} show_data={show_data}/>

      <div className="flex gap-3 w-full">
        <input
          type="text"
          defaultValue={show_data ? firstName : ""}
          name="firstName"
        ...
        <input
          type="text"
          defaultValue={show_data ? lastName : ""}
          name="lastName"
      ...
      <input
        type="text"
        defaultValue={show_data ? address : ""}
        name="address"
        placeholder="Address"
        required
        maxLength={20}
      ...
      <div className="flex gap-3 w-full">
        ...
        <input
          type="number"
          defaultValue={show_data ? postal_code : ""}
          name="postalCode"
      ...
      <SubmitButton
        ...
        styles="text-[15px] text-white bg-black w-[152px] h-[56px] flex justify-center items-center rounded-[10px] shadow-[0_20px_35px_0_rgba(0,0,0,0.15)] focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-black w-full"
      >
        Pay Now
      </SubmitButton>
